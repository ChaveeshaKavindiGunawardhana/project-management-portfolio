# project-management-portfolio
Project Management documentation, tools, and certifications 
# Hi, I'm P.H.M.C.K. Gunawardhana

## Project Management Student | HNDPM

Pursuing a Higher National Diploma in Project Management (HNDPM) at 
Sri Lanka Institute of Advanced Technological Education, Kegalle.

### 🎓 Core Project Management Certifications
- PMI: Introduction to Project Management Professional (PMP)
- PMI Essentials: M.O.R.E. Maximizing Project Success
- PMI Fundamentals of Agile Project Management
- PMI Fundamentals of Predictive Project Management
- Diploma in Modern Project Management (Alison)

### 🛠️ Tools & Technical Skills
- Microsoft Project Training (Master of Project Academy)
- Primavera P6 Fundamentals (Alison)
- Introduction to JIRA (Simplilearn)
- Basics of Microsoft Power BI (UniAthena)
- Introduction to Data Analytics (Simplilearn)
- Cisco Data Analytics Essentials

### 📊 Quality & Process Management
- Lean Six Sigma: White Belt (Alison)
- Basics of TQM – Deployment & Auditing (UniAthena)

### 🤖 AI in Project Management
- PMI Generative AI Overview for Project Managers
- PMI Essentials: Seven AI Project Patterns

### 💼 Practical Experience
- Project Management Workshop (Ministry of Transport, Highways & 
  Urban Development project) - Makumbura Multimodal Center

 ### 📁 Project Management Templates
- [Project Schedule / Gantt Chart Template](templates/Project-Schedule-Gantt-Template.xlsx)
- [Budget Tracker & Risk Register Template](templates/Budget-Risk-Register-Template.xlsx) 

### 📈 Data Analytics Projects
See the [data-analytics](data-analytics/) folder for Excel dashboards 
and analytical work.

### Contact
Email: kavindigunawardhana931@gmail.com
